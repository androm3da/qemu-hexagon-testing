...
static void hvx_conv3x3_f16(const __fp16 *restrict padded, uint32_t width,
                            uint32_t height, const float *restrict mask,
                            __fp16 *restrict output)
{
    int16_t mask_bits[9];

    /*
     * FP32-to-FP16 conversion is a call on Hexagon.  Convert all taps before
     * creating the HVX accumulator so it never has to survive that call.
     */
    for (uint32_t i = 0; i < 9; i++) {
        __fp16 tap = (__fp16)mask[i];
        mask_bits[i] = float16_to_bits(&tap);
    }
