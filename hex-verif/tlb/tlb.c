...
// some devices have 128 tlb entries, for this test 63 is plenty
...
void print_tlbs(void)
{
	...
	printf("\nIdx:  VPAGE    PPAGE       SIZE   ASID  R W X U  CCCC  S  V  G  EP  A1 A0\n");
	...
	for(tlbnum = 0; tlbnum <= NUM_TLB_ENTRIES; tlbnum++)
	{
		...
		if(tlb_entries[tlbnum] != 0x00)
		{
			...
			printf("%4d  0x%05x  0x%06x    %-5s  0x%02x  %d %d %d %d  0x%0x   %d  %d  %d  %d   %d  %d\n", tlbnum, VIRT_PAGE(tlbhi), PPN, PAGE_SIZE(tlblo, mask), ASID(tlbhi),
					P_READ(tlblo), P_WRITE(tlblo), P_EXEC(tlblo), P_USER(tlblo), P_CCCC(tlblo), P_S(tlblo), P_V(tlbhi), P_G(tlbhi), P_EP(tlbhi), P_A1(tlbhi), P_A0(tlbhi));
		}
	}
}
