...
use crate::recipe::{Recipe, RecipeFilters, SynthSettings};
...
pub struct SynthesisConfig {
    ...
    ///
    /// When enabled, simple base+offset memory operations are synthesized
...
"#;
        ...
        assert!(recipe.synth.allow_mem_ops);
    ...
    #[test]
    fn test_allow_mem_ops_from_toml() {
        let toml_str = r#"
[synthesis]
allow_mem_ops = true
"#;
    ...
    #[test]
    fn test_allow_mem_ops_default_false() {
        let toml_str = "";
        let recipe_file: RecipeFile = toml::from_str(toml_str).unwrap();
        let recipe = recipe_file.into_recipe(42).unwrap();
        assert!(!recipe.synth.allow_mem_ops);
