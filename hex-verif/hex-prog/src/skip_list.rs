...
pub const DEFAULT_SKIP_TERMS: &[&str] = &[
    "mem",
    "swi(", // "swi(" avoids false-positive on A2_swiz (byte-swap)
    "trap",
    "scatter",
    "gather",
    ".new",
    ...
    "dczero",
    "resume",
    // System-level TypeCR operations (dangerous for random synthesis)
    "ciad",
    "siad",
    "crswap",
    "getimask",
    ...
    "setimask",
    "trace(",
    // Register transfers to/from control/guest/system registers
    "tfrcrr",
    "tfrrcr",
    "tfrcpp",
    "tfrpcp",
...
mod tests {
    ...
    #[test]
    fn test_swi_paren_matches_swi_insn() {
        // "swi(" should match the software interrupt instruction
        assert!(matches_skip_list(
            "Y2_swi",
            "swi($Rs32)",
            DEFAULT_SKIP_TERMS
    ...
    #[test]
    fn test_swiz_not_matched_by_swi() {
        // "swi(" should NOT match A2_swiz (byte-swap), avoiding false positive
        assert!(!matches_skip_list(
            "A2_swiz",
            "$Rd32 = swiz($Rs32)",
            DEFAULT_SKIP_TERMS
