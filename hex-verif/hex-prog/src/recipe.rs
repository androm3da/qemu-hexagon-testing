...
/// Controls how the generated program terminates its `steps` function.
///
/// In **Direct** mode (the default), `steps` falls through into a register
/// dump that writes state to stdout, followed by `call exit`. Each emulator
/// launch produces one register snapshot.
///
/// In **Gdb** mode, `steps` returns via `jumpr r31` and the caller (`body`)
/// loops `num_iterations` times, hitting a `check_point` breakpoint after
/// each call. The GDB client reads registers at each breakpoint, giving N×
/// more coverage per emulator launch.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum ExecutionMode {
    /// Stdout-based register dump at test_end (fast, single snapshot).
    #[default]
    Direct,
    /// GDB breakpoint-based register reads (iterating, multiple snapshots).
    Gdb,
}
...
pub struct Recipe {
    ...
    pub hvx: bool,
    /// Execution mode — controls how `steps` terminates and how `body` loops.
    ///
    /// Set by the runner based on `--mode`; not loaded from TOML.
...
///   and branch types (TypeJ, TypeCJ, TypeNCJ)
...
/// | `allow_memory_ops` | `false` | Load/store instructions disabled |
/// | `allow_jumps` | `false` | Jump instructions disabled |
/// | `forward_only_jumps` | `true` | Jumps only go forward |
/// | `allow_pageable` | `false` | Pageable memory (TLB miss + replay) disabled |
///
/// # Examples
///
/// ```
...
pub struct SynthSettings {
    ...
    ///
    /// When `true`, `"mem"` is automatically removed from skip terms and
    /// setup packets are emitted before each packet containing memory ops
    /// to initialize base registers into `mem_region`.
    pub allow_memory_ops: bool,
    /// Allow jump instructions.
    ///
    /// When `true`, `"jump"` is automatically removed from skip terms and
    /// branch types (TypeJ, TypeCJ, TypeNCJ) are un-excluded. Jump targets
    /// are replaced with step labels.
    pub allow_jumps: bool,
    /// When `true`, jumps only target forward labels (toward `test_end`),
    /// guaranteeing termination. When `false`, backward jumps are allowed,
    /// which may create loops (requires timeout).
    pub forward_only_jumps: bool,
    /// Enable pageable memory for TLB miss + packet replay testing.
    ///
    /// When `true`, a separate memory region is mapped via the software
    /// page table at a VA outside the default TLB. Memory ops target this
    /// region (via r26) instead of `mem_region` (r27), causing a TLB miss
    /// on the first access each iteration. The fault handler fills the TLB
    /// from the PTE and the CPU replays the entire packet.
    ///
    /// Implies `allow_memory_ops = true`. Adds `"r26"` to skip terms to
    /// protect the pageable base register from the synthesizer.
    pub allow_pageable: bool,
}
...
impl Default for SynthSettings {
    fn default() -> Self {
        Self {
            ...
            allow_jumps: false,
            forward_only_jumps: true,
            allow_pageable: false,
        }
    }
}
